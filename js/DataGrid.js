...
class DataGrid {
    ...
    applyFilters() {
        ...
        this.filteredData = this.data.filter(item => {
            ...
            if (this.filters.ustMakineGrubu && this.filters.ustMakineGrubu !== '' && this.filters.bolum) {
                ...
                if (ustMakineGruplari) {
                    ...
                    if (makinelerInGroup.length > 0) {
                        const selectedUstMakineGrubuTrimmed = selectedUstMakineGrubu.trim();
                        
                        // 1. Ana kayıt makine kontrolü - alt makinelere tanımlı mı?
                        const mainMachineInGroup = makinelerInGroup.some(m => 
                            m.trim().toLowerCase() === (item.makAd || '').trim().toLowerCase()
                        );
                        
                        // 2. Ana kayıt makine kontrolü - direkt üst makineye tanımlı mı?
                        const mainMachineIsUpper = (item.makAd || '').trim().toLowerCase() === selectedUstMakineGrubuTrimmed.toLowerCase();
                        ...
                        if (item.breakdowns && item.breakdowns.length > 0) {
                            breakdownMachineInGroup = item.breakdowns.some(breakdown => {
                                ...
                                const breakdownMakAdTrimmed = breakdownMakAd.trim();
                                // Alt makine kontrolü
                                const isSubMachine = makinelerInGroup.some(m => 
                                    m.trim().toLowerCase() === breakdownMakAdTrimmed.toLowerCase()
                                );
                                // Üst makine kontrolü
                                const isUpperMachine = breakdownMakAdTrimmed.toLowerCase() === selectedUstMakineGrubuTrimmed.toLowerCase();
                        ...
                        const selectedMachineInGroup = item.selectedMachine && (
                            makinelerInGroup.some(m => 
                                m.trim().toLowerCase() === item.selectedMachine.trim().toLowerCase()
                            ) || item.selectedMachine.trim().toLowerCase() === selectedUstMakineGrubuTrimmed.toLowerCase()
                        );
            ...
            if (this.filters.makina && this.filters.makina !== '') {
                const selectedMachine = this.filters.makina.trim();
                // Ana kayıt makine kontrolü (case-insensitive ve trim ile)
                const mainMachineMatch = item.makAd && 
                    item.makAd.trim().toLowerCase() === selectedMachine.toLowerCase();
                // Breakdown'lardaki makine kontrolü
                let breakdownMachineMatch = false;
                if (item.breakdowns && item.breakdowns.length > 0) {
                    breakdownMachineMatch = item.breakdowns.some(breakdown => {
                        const breakdownMakAd = breakdown.makAd || breakdown.selectedMachine;
                        return breakdownMakAd && 
                            breakdownMakAd.trim().toLowerCase() === selectedMachine.toLowerCase();
                    });
                }
                // selectedMachine kontrolü
                const selectedMachineMatch = item.selectedMachine && 
                    item.selectedMachine.trim().toLowerCase() === selectedMachine.toLowerCase();
    ...
    /**
     * Bölüm için tüm makineleri üst makine gruplarına göre gruplayarak döndürür
     * @param {string} bolumAdi - Bölüm adı
     * @param {string} makAd - Mevcut makine adı (üst makine grubu kontrolü için)
    ...
    async getMachinesWithGroupsForBolum(bolumAdi, makAd = '') {
        ...
        // Mapping'den üst makine gruplarını kontrol et (bölüm adı eşleşmesi için esnek kontrol)
        let bolumMapping = this.machineMapping[bolumAdi];
        
        // Eğer direkt eşleşme yoksa, mapping anahtarlarını kontrol et
        if (!bolumMapping && bolumAdi) {
            // Önce normalize edilmiş karşılaştırma yap
            const normalizedBolumAdi = bolumAdi.replace(/[^A-Za-zÇĞİÖŞÜçğıöşü]/g, '').toLowerCase();
            
            Object.keys(this.machineMapping).forEach(mappingKey => {
                const normalizedMappingKey = mappingKey.replace(/[^A-Za-zÇĞİÖŞÜçğıöşü]/g, '').toLowerCase();
                
                // Bölüm adı mapping anahtarıyla eşleşiyor mu kontrol et
                if (bolumAdi.includes(mappingKey) || mappingKey.includes(bolumAdi) || 
                    normalizedBolumAdi === normalizedMappingKey ||
                    normalizedBolumAdi.includes('maça') && normalizedMappingKey.includes('maça')) {
                    bolumMapping = this.machineMapping[mappingKey];
                    console.log('✅ Mapping bulundu (esnek eşleşme):', { bolumAdi, mappingKey });
                }
            });
        }
        ...
        // Debug: Bölüm adı ve mapping kontrolü
        console.log('🔍 getMachinesWithGroupsForBolum:', {
            bolumAdi,
            makAd,
            isMacaBolumu,
            hasMapping: !!bolumMapping,
            mappingKeys: bolumMapping ? Object.keys(bolumMapping) : [],
            bolumMachinesFromDB: bolumMachines
        });
        ...
        if (bolumMapping) {
            ...
            console.warn('⚠️ Mapping bulunamadı! Bölüm adı:', bolumAdi, 'Mapping anahtarları:', Object.keys(this.machineMapping));
            // Mapping yoksa, hiçbir şey gösterme (çünkü gruplama yapamayız)
            // Eğer sadece tek bir makine varsa onu göster
            if (bolumMachines.length === 1 && makAd && bolumMachines[0] === makAd) {
                machines.push({ makAd: makAd, group: 'Makineler' });
                machineGroups['Makineler'] = [makAd];
            } else if (bolumMachines.length > 0) {
                // Mapping yoksa ama makineler varsa, basit bir grup oluştur
                machines.push(...bolumMachines.map(m => ({ makAd: m, group: 'Makineler' })));
                machineGroups['Makineler'] = bolumMachines;
            } else if (makAd) {
                machines.push({ makAd: makAd, group: 'Makineler' });
                machineGroups['Makineler'] = [makAd];
            }
        ...
        return { machines, groups: machineGroups };
    }
        ...
        for (const select of machineSelects) {
            ...
            try {
                ...
                if (orderBolumAdi) {
                    ...
                    // Bölüm bilgisi yoksa, sadece mevcut makineyi göster (grup olmadan)
                    if (orderMakAd) {
                        machines = [{ makAd: orderMakAd, group: 'Makineler' }];
                        machineGroups['Makineler'] = [orderMakAd];
                    }
                }
                ...
                if (machines.length === 0) {
                    ...
                    const defaultMachine = currentMachine || orderMakAd || machines[0].makAd;
        ...
        for (const select of machineSelects) {
            ...
            try {
                ...
                if (bolumAdi) {
                    ...
                    machineGroups['Makineler'] = [stage.makAd];
                }
                ...
                if (machines.length === 0) {
                    ...
                    // Default makineyi belirle
                    const defaultMachine = currentMachine || stage.makAd || machines[0].makAd;
    ...
    async addMachineSelectionForAllDepartments(modal, item) {
        ...
        try {
            ...
            // Tüm bölümler için (maça dahil) bölüm makinelerini al
            let machines = [];
            let machineGroups = {}; // Üst makine gruplarına göre gruplanmış makineler
            let machineInfo = null;
            
            // Bölüm makinelerini al (üst makine gruplarına göre gruplanmış)
            // Üst makine grubuna atanmış olsa bile, TÜM bölüm makinelerini göster
            if (bolumAdi) {
                // makAd parametresini boş göndererek tüm grupları göster
                // (Üst makine grubuna atanmış işler için de tüm makineler gösterilmeli)
                const result = await this.getMachinesWithGroupsForBolum(bolumAdi, '');
                machines = result.machines;
                machineGroups = result.groups;
    ...
    async openUpperMachinePlanningModal(item, modal, machineInfo) {
        ...
        // addMachineSelectionForAllDepartments kullanarak tüm maça makinelerini (tüm gruplarıyla) göster
        await this.addMachineSelectionForAllDepartments(modal, item);
     ...
     */
    async updateMachineSelectionOptions(machineField, machines, availabilityData, defaultMachine, selectedDate, machineGroups = null) {
        const machineSelect = machineField.querySelector('#machineSelection');
        ...
        // Üst makine gruplarına göre makineleri grupla
        let groupedMachines = machineGroups || {};
        
        // Eğer machineGroups verilmemişse, machines array'inden grupla
        if (!machineGroups || Object.keys(machineGroups).length === 0) {
            groupedMachines = {};
            machines.forEach((subMachine) => {
                const machineName = typeof subMachine === 'string' ? subMachine : subMachine.makAd;
                const group = typeof subMachine === 'string' ? 'Makineler' : (subMachine.group || 'Makineler');
                if (!groupedMachines[group]) {
                    groupedMachines[group] = [];
                }
                groupedMachines[group].push(machineName);
            });
        }
        
        // Optgroup'lar ile dropdown oluştur
        let options = '';
        Object.keys(groupedMachines).sort().forEach(groupName => {
            // Üst makine grubu başlığı (optgroup - seçilemez)
            options += `<optgroup label="${groupName}">`;
            
            // Bu grubun altındaki makineler
            groupedMachines[groupName].forEach(machineName => {
        ...
        for (const select of machineInputs) {
            ...
            try {
                ...
                if (machines.length === 0) {
                    ...
                    // Default makineyi belirle
                    const defaultMachineValue = item.selectedMachine || item.makAd || machines[0].makAd;
                    ...
                    Object.keys(machineGroups).sort().forEach(groupName => {
                        ...
                        // Bu grubun altındaki makineler
                        machineGroups[groupName].forEach(machineName => {
                            const isSelected = defaultMachineValue === machineName;
                            select.innerHTML += `<option value="${machineName}" ${isSelected ? 'selected' : ''}>${machineName}</option>`;
                        });
